 ...
 ***************************************************************************/
int
WindowPrompt(const char *title, const char *msg, const char *btn1Label, const char *btn2Label, GuiWindow *parentWindow)
{
	int choice = -1;
  ...
  GXColor helpTxtColor = (GXColor){0, 170, 0, 255};
	GuiText titleTxt(title, 24, helpTxtColor);
   ...
   if (!parentWindow)
	...
	parentWindow->Append(&promptWindow);
	parentWindow->ChangeFocus(&promptWindow);
	ResumeGui();
	...
	HaltGui();
	parentWindow->Remove(&promptWindow);
...
/*static int MenuSettings()
{
	...
	GuiText exitBtnTxt("TetriCycle", 22, (GXColor){0, 0, 0, 255});
